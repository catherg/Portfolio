...
class BirdSystem extends ParticleSystem {
  static label = "🦅"; // Ignore the Glitch parse error
  static desc = "Some very confused birds"; // Ignore the Glitch parse error

  constructor() {
    // Make what particle, and how many?
    // Try different numbers of particles
    super(BoidParticle, 200);
  ...
  draw(p) {
    p.background(200, 150, 60, 10);

    p.noFill();
    p.stroke(200, 0, 100, 0.4);
...
class BoidParticle extends Particle {
  ...
  calculateForces(p, dt) {
    ...
    // Add a border force
    let center = new Vector2D(p.width / 2 + p.width / 4, p.height / 2);
    this.f.add(
    ...
    this.attractionForce = this.pos.getForceTowardsPoint(mouse, 1, {falloff: 1.3})
    ...
    this.separationForce.mult(.3)
    this.cohesionForce.mult(.5)
    this.alignmentForce.mult(.4)
  ...
  draw(p, drawDebug = false) {
    ...
    p.vertex(-this.radius, this.radius);
    */
    p.text(emoji, -this.radius, -this.radius);
    ...
    if (drawDebug) {
      ...
      p.text(this.debugText, ...this.pos)
       this.pos.drawArrow(p, this.separationForce, { m: .1, color: [30, 100, 50] });
      this.pos.drawArrow(p, this.cohesionForce, { m: .01, color: [60, 100, 50] });
