...
class WindParticle extends Particle {
  ...
  calculateForces(p, dt) {
    ...
    windDir = 40*p.noise(windX, windY, t*.1)
  ...
  draw(p, drawDebug = false) {
    ...
    p.fill(
      this.hue,
      50 + 80 * p.noise(this.idNumber + t + 50),
      30 + 80 * p.noise(this.idNumber + t + 100)
    );
    ...
    let leafLength = 20
    p.beginShape();
    p.vertex(-leafLength*.1, 0);
    p.curveVertex(leafLength*-.5, -leafWidth * .1);
    p.vertex(leafLength*.2, 0);
    p.curveVertex(leafLength*-.8, leafWidth * .2);
    p.vertex(-leafLength*.3, 0);
    p.vertex(leafLength*.4, 0);
    p.curveVertex(leafLength*-.3, leafWidth * .3);
    p.vertex(-leafLength*.5, 0);
    p.curveVertex(leafLength*-.5, -leafWidth * .4);
    p.vertex(leafLength*.6, 0);
    p.curveVertex(leafLength*-.8, leafWidth * .5);
    p.vertex(-leafLength*.7, 0);
    p.vertex(leafLength*.8, 0);
    p.curveVertex(leafLength*-.9, leafWidth * .6);
    p.vertex(leafLength*.9, 0);
    p.curveVertex(leafLength*-.9, leafWidth * .9);
    p.vertex(leafLength*.99, 0);
    p.curveVertex(leafLength*-.9, leafWidth * .999);
    p.endShape(p.CLOSE);
    p.pop();
